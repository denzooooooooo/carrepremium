            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 sticky top-24">
                <nav className="space-y-2">
                  {[
                    { id: 'info', icon: 'fa-user', label: t('personalInfo', 'Informations') },
                    { id: 'security', icon: 'fa-lock', label: t('security', 'Sécurité') },
                    { id: 'stats', icon: 'fa-chart-line', label: t('statistics', 'Statistiques') }
                  ].map(tab => (
                  ...
                  <Link to="/my-bookings" className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    <i className="fas fa-ticket-alt"></i><span>{t('myBookings', 'Mes Réservations')}</span>
                  </Link>
                  <button onClick={logout} className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                    <i className="fas fa-sign-out-alt"></i><span>{t('logout', 'Déconnexion')}</span>
                  </button>
                </nav>
              </div>
            </div>
            ...
            <div className="lg:col-span-3">
              ...
              {activeTab === 'info' && (
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                  <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-6">{t('personalInfo', 'Informations Personnelles')}</h2>
                  <form onSubmit={handleProfileSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {['first_name', 'last_name'].map(field => (
                        <div key={field}>
                          <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                            {t(field, field === 'first_name' ? 'Prénom' : 'Nom')}
                          </label>
                          <input type="text" name={field} value={profileData[field]} onChange={handleProfileChange} className="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-purple-600 focus:ring-0" />
                        </div>
                      ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('phone', 'Téléphone')}</label>
                      ...
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('dateOfBirth', 'Date de naissance')}</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('gender', 'Genre')}</label>
                        <select name="gender" value={profileData.gender} onChange={handleProfileChange} className="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-purple-600 focus:ring-0">
                          <option value="">{t('selectGender', 'Sélectionner')}</option>
                      ...
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('nationality', 'Nationalité')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('passportNumber', 'Numéro de passeport')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('address', 'Adresse')}</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {['city', 'country', 'postal_code'].map(field => (
                        <div key={field}>
                          <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                            {t(field, field === 'city' ? 'Ville' : field === 'country' ? 'Pays' : 'Code postal')}
                          </label>
                          <input type="text" name={field} value={profileData[field]} onChange={handleProfileChange} className="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-purple-600 focus:ring-0" placeholder={field === 'city' ? 'Abidjan' : field === 'country' ? 'Côte d\'Ivoire' : '00225'} />
                        </div>
                      ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('preferredLanguage', 'Langue préférée')}</label>
                      ...
                      <div>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{t('preferredCurrency', 'Devise préférée')}</label>
                    ...
                    <button type="submit" disabled={loading} className="w-full py-4 bg-gradient-to-r from-purple-600 to-purple-700 text-white font-bold rounded-xl hover:from-purple-700 hover:to-purple-800 transition-all duration-300 disabled:opacity-50 shadow-lg hover:shadow-2xl">
                      {loading ? <><i className="fas fa-spinner fa-spin mr-2"></i>{t('saving', 'Enregistrement...')}</> : <><i className="fas fa-save mr-2"></i>{t('saveChanges', 'Enregistrer les modifications')}</>}
                    </button>
                  </form>
                </div>
              )}
              ...
              {activeTab === 'security' && (
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                  <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-6">{t('securitySettings', 'Paramètres de Sécurité')}</h2>
                  <form onSubmit={handlePasswordSubmit} className="space-y-6">
                    {['current_password', 'new_password', 'new_password_confirmation'].map(field => (
                      <div key={field}>
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                          {t(field, field === 'current_password' ? 'Mot de passe actuel' : field === 'new_password' ? 'Nouveau mot de passe' : 'Confirmer le nouveau mot de passe')}
                        </label>
                        <input type="password" name={field} value={passwordData[field]} onChange={handlePasswordChange} required className="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-purple-600 focus:ring-0" placeholder="••••••••" />
                      </div>
                    ))}
                    <button type="submit" disabled={loading} className="w-full py-4 bg-gradient-to-r from-purple-600 to-purple-700 text-white font-bold rounded-xl hover:from-purple-700 hover:to-purple-800 transition-all duration-300 disabled:opacity-50 shadow-lg hover:shadow-2xl">
                      {loading ? <><i className="fas fa-spinner fa-spin mr-2"></i>{t('updating', 'Mise à jour...')}</> : <><i className="fas fa-key mr-2"></i>{t('changePassword', 'Changer le mot de passe')}</>}
                    </button>
                  </form>
                </div>
              )}
              ...
              {activeTab === 'stats' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                      { icon: 'fa-ticket-alt', color: 'purple', value: statistics?.total_bookings || 0, label: t('totalBookings', 'Réservations') },
                      { icon: 'fa-check-circle', color: 'green', value: statistics?.confirmed_bookings || 0, label: t('confirmedBookings', 'Confirmées') },
                      { icon: 'fa-wallet', color: 'amber', value: formatPrice(statistics?.total_spent || 0), label: t('totalSpent', 'Dépensé') },
                      { icon: 'fa-star', color: 'yellow', value: statistics?.loyalty_points || 0, label: t('loyaltyPoints', 'Points') }
                    ].map((stat, idx) => (
                  ...
                  {/* Bookings by Type */}
                  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t('bookingsByType', 'Réservations par Type')}</h3>
                    <div className="space-y-4">
                      {statistics?.bookings_by_type && Object.entries(statistics.bookings_by_type).map(([type, count]) => (
                        <div key={type} className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <i className={`fas ${type === 'flight' ? 'fa-plane' : type === 'event' ? 'fa-ticket-alt' : 'fa-suitcase'} text-purple-600`}></i>
                            <span className="font-semibold text-gray-700 dark:text-gray-300">
                              {type === 'flight' ? 'Vols' : type === 'event' ? 'Événements' : 'Packages'}
                            </span>
                          </div>
                  ...
                  {statistics?.recent_bookings && statistics.recent_bookings.length > 0 && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t('recentBookings', 'Réservations Récentes')}</h3>
                      <div className="space-y-4">
                        {statistics.recent_bookings.map((booking) => (
                          <Link key={booking.id} to={`/my-bookings/${booking.id}`} className="block p-4 border-2 border-gray-200 dark:border-gray-700 rounded-xl hover:border-purple-600 transition-colors">
